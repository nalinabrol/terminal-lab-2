...
| Page | Activity | Exercise plan | Minutes |
|---|---|---|---|
| 1 | Setup | Launch the separate Codespace, inspect supplied folders, understand originals versus working copies. | 5 |
| 2 | History and search | Type harmless starter commands; inspect `history` and `history 10`; filter with `grep -F`; use Ctrl+R and repeat for older matches; recall and edit before execution; cancel with Ctrl+G. Independently adapt an earlier search. | 8 |
| 3 | nano | Correct the room and time in a notice, add a missing instruction, save, exit, reopen, then make an independent correction. | 10 |
| 4 | less | Navigate a long handbook, jump to beginning/end, search and advance between matches, find the applicable instruction, then quit. | 8 |
| 5 | sort | Compare alphabetical and numeric order, reverse numeric order, save a highest-to-lowest score list. | 10 |
| 6 | uniq | Compare adjacent and separated duplicates; sort before deduplication; count occurrences; produce unique names and a frequency report. | 10 |
| 7 | cut | Identify a pipe delimiter and field positions; extract one or multiple fields; combine extraction, sorting, and duplicate counts to summarise workshop choices. | 10 |
...
- Simple, concrete tasks. Lab 1 topics recur only as supporting steps or clear extensions.
- No permissions, environment-variable lessons, processes, substantive scripting, or archives.
- History introduces more than Lab 1's Up Arrow recall; omit `!!` and `!number` for now.
- Use consistent delimiter-separated data without embedded delimiters or CSV quoting complications.
